 ...
 * @author nhatc
 */
public class KhoaiTV {

//    private final String xPathGetLinkShowAll = "//*[@id='menu']//div/ul/li[3]/a/@href";
//    private final String XpathGetLinkEachItem = "//*[@class='list-film row']/div[contains(@class,'item')]/div/a";
//    private final String xpathGetNameFilm = "//*[contains(@class,'info-film')]//h2/text()";
//    private final String xpathGetNameFilmEnglish = "//*[contains(@class,'info-film')]//div[@class='name2']/dfn/text()";
////    private final String xpathGetPagination = "//*[local-name()='svg']";
//    private String host = "https://24hphim.com";
//    private String hostName = "";
    ...
    public KhoaiTV(String realPath) {
        this.realPath = realPath;
        this.crawlerConfig = XMLUtilities.readXMLCrawlerConfig(realPath + "/" + Constant.CRAWLER_CONFIG_FOLDER + "/" + Constant.PHIM24H_FILE_NAME);
    }
    ...
    public void testValidateAndInsertDB() {
        try {
            ...
            this.listFilm = unmarshallingListFilm(realPath + "test/test.xml");
            validateData();
...
//                XMLUtilities.saveToXML(realPath + "/test/24hPhim.xml", listFilm);
...
//                System.out.println("web loi");
             ...
             Logger.getLogger(KhoaiTV.class.getName()).log(Level.SEVERE, null, "Loi cau truc config");
        }
        return listFilm;
    }
...
//    }
    public boolean validateData() {
        ...
        XMLUtilities.saveToXML(this.realPath + Constant.VALIDATED_FOLDER + "24hphimValid.xml", new ListFilm(validFilms));
        XMLUtilities.saveToXML(this.realPath + Constant.VALIDATED_FOLDER + "24hphimInvalid.xml", new ListFilm(invalidFilms));
    ...
    public int getPagination(String uri) {
        ...
        try {
            ...
            NodeList listHref = (NodeList) xpath.evaluate(exp, doc, XPathConstants.NODESET);
            exp = "./a/@href";
            String href = (String) xpath.evaluate(exp, listHref.item(listHref.getLength() - 1), XPathConstants.STRING);
            String lastPage = href.substring(href.lastIndexOf('/') + 1);
            lastPage = TextUtilities.getOnlyNumber(lastPage);
//            String lastPagi = pagiNodelist.item(pagiNodelist.getLength() - 1).getTextContent();
            pagi = Integer.parseInt(lastPage);
            return pagi;
        } catch (Exception e) {
...
//                System.out.println("engName:  " + engName);
                ...
                for (int i = 0; i < infoRows.getLength(); i++) {
                    ...
                    if (title.contains(crawlerConfig.getDirectorKeyword())) {
                        exp = crawlerConfig.getXpathDirector() + "[" + position + "]" + "/a";
...
//                                System.out.println(director.trim());
                    ...
                    if (title.contains(crawlerConfig.getActorKeyword())) {
                        // get actor
                        exp = crawlerConfig.getXpathActor() + "[" + position + "]" + "/a";
...
//                                System.out.println(actors.item(j).getTextContent());
                    ...
                    if (title.contains(crawlerConfig.getKindKeyword())) {
                        // get kind
                        exp = crawlerConfig.getXpathKind() + "[" + position + "]" + "/a";
...
//                                System.out.println(kind.trim());
                    ...
                    if (title.contains(crawlerConfig.getNationKeyword())) {
                        // get nation
                        exp = crawlerConfig.getXpathNation() + "[" + position + "]" + "/a/text()";
...
//                        System.out.println("nation: " + national);
                    ...
                    if (title.contains(crawlerConfig.getDurationKeyword())) {
                        //get duration
                        exp = crawlerConfig.getXpathDuration() + "[" + position + "]";
...
//                        System.out.println("duration: " + duration);
                    ...
                    if (title.contains(crawlerConfig.getYearKeyword())) {
                        // get time
                        exp = crawlerConfig.getXpathYear() + "[" + position + "]" + "/a/text()";
                        String yearNode = (String) xpath.evaluate(exp, infoRows.item(i), XPathConstants.STRING);
//                        String year = TextUtilities.getOnlyNumber(yearNode.getTextContent());
                        String year = TextUtilities.getOnlyNumber(yearNode);
//                        System.out.println("year: " + year);
                ...
                String views = (String) xpath.evaluate(exp, doc, XPathConstants.STRING);
                views = TextUtilities.getOnlyNumber(views);
                if (film != null) {
                    film.setNumberOfView(Integer.parseInt(views));
                } else {
                    film.setNumberOfView(0);
                }
                //end get view 
                // get rate
                exp = crawlerConfig.getXpathRate();
                String rate = (String) xpath.evaluate(exp, doc, XPathConstants.STRING);
